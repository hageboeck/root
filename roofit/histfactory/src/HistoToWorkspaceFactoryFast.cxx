...
RooArgList HistoToWorkspaceFactoryFast::createObservables(const TH1 *hist, RooWorkspace *proto) const {
  ...
  RooHistFunc* HistoToWorkspaceFactoryFast::MakeExpectedHistFunc(const TH1* hist,RooWorkspace* proto, string prefix,
    ...
    RooDataHist histDHist((prefix+"nominalDHist").c_str(),"",observables,hist);
    RooHistFunc histFunc((prefix+"_nominal").c_str(),"",observables,histDHist,0);

    proto->import(histFunc);
    auto histFuncInWS = static_cast<RooHistFunc*>(proto->arg((prefix+"_nominal").c_str()));
  ...
  RooAbsArg* HistoToWorkspaceFactoryFast::MakeLinInterpWithConstraint(RooHistFunc* nominalFunc,
      RooWorkspace* proto, std::vector<HistoSys> histoSysList,
      string prefix, vector<string>& constraintTermNames,
      const RooArgList& observables) const {
  ...
  void  HistoToWorkspaceFactoryFast::MakeTotalExpected(RooWorkspace* proto, string totName, 
						       vector<RooProduct*>& sampleScaleFactors, std::vector<vector<RooAbsArg*>>& sampleHistFuncs) const {
    ...
    RooBinWidthFunction binWidth(binWidthFunctionName.c_str(), "Multiply bins contents with bin width", *firstHistFunc);
    proto->import(binWidth);
    auto binWidthWS = proto->function(binWidthFunctionName.c_str());
    assert(binWidthWS);

#pragma message("Do we need to multiply with the bin width?")
    ...
    for (unsigned int i=0; i < sampleHistFuncs.size(); ++i) {
      ...
      sampleHistFuncs[i].push_back(binWidthWS);

      if (sampleHistFuncs[i].size() == 1) {
        shapeList.add(*sampleHistFuncs[i].front());
      } else {
        std::string name = sampleHistFuncs[i].front()->GetName();
        auto pos = name.find("nominal");
        if (pos != std::string::npos) {
          name = name.substr(0, pos) + "shapes";
        }

        RooArgSet terms;
        for (auto func : sampleHistFuncs[i]) {
          terms.add(*func);
        }

        RooProduct shapeProduct(name.c_str(), name.c_str(), terms);
        proto->import(shapeProduct);
    ...
    // The bin width function will be imported multiple times, so recycle here:
    proto->import(tot);
  }
  ...
  RooWorkspace* HistoToWorkspaceFactoryFast::MakeSingleChannelWorkspace(Measurement& measurement, Channel& channel) {
    ...
    for (Sample& sample : channel.GetSamples()) {
      ...
      // GHL: Consider passing the NormFactor list instead of the entire sample
      auto sampleProduct = CreateNormFactor(proto, channel_name, overallSystName, sample, doRatio);
...
#pragma message("Multiply with bin width here")
      ...
      if( sample.GetStatError().GetActivate() ) {
        ...
        if( fObsNameVec.size() > 3 ) {
          ...
          if( paramHist == nullptr ) {
            ...
            // Multiply the paramHistFunc with the existing samples
            for (auto& funcs : allSampleHistFuncs) {
              funcs.push_back(paramHist);
            }

          } else {
            // stat function exists, apply only to latest sample
      ...
      // RooRealSumPdf
      proto->import(*sampleProduct, RecycleConflictNodes());
      auto sampleProductinWS = dynamic_cast<RooProduct*>(proto->arg(sampleProduct->GetName()));
      assert(sampleProductinWS);

      sampleScaleFactors.push_back(sampleProductinWS);
