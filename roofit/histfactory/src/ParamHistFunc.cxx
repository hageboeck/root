					     ...
					     const char* /*rangeName*/) const 
{
  ...
  auto binVolumes = _dataSet.binVolumes();

  for (unsigned int i=0; i < _paramSet.size(); ++i) {
    auto& param = static_cast<const RooRealVar&>(_paramSet[i]);

    // Get the gamma's value
    const double paramVal  = param.getVal();
    
    // Get the bin volume
    const double binVolumeDS  = _dataSet.binVolume(); //_binning->binWidth( nominalItr );
